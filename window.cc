...
void Xwindow::drawBigString(int x, int y, string msg, int colour) {
  ...
  ostringstream name;
  name << "-*-helvetica-bold-r-*-*-*-240-" << width/4 << "-" << height/5 << "-*-*-*-*";
...
void Xwindow::drawBigString2(int x, int y, string msg, int colour) {
  XSetForeground(d, gc, colours[colour]);
  ...
  // Font f = XLoadFont(d, "-*-helvetica-bold-r-normal--*-240-*-*-*-*-*");
  ostringstream name;
  name << "-*-helvetica-bold-r-*-*-*-240-" << width/8 << "-" << height/9 << "-*-*-*-*";
...
void Xwindow::showAvailableFonts() {
  int count;
  char** fnts = XListFonts(d, "*", 100, &count);
